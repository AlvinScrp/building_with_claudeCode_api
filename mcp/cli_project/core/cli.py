...
class CliApp:
    ...
    async def refresh_resources(self):
        try:
            ...
            _logger.info("refresh_resources: count=%d", len(self.resources))
        except Exception as e:
            _logger.exception("Error refreshing resources: %s", e)
    ...
    async def refresh_prompts(self):
        try:
            ...
            _logger.info("refresh_prompts: count=%d", len(self.prompts))
        except Exception as e:
            _logger.exception("Error refreshing prompts: %s", e)

    async def run(self):
        _logger.info("run: CLI loop started")
        while True:
            try:
                ...
                _logger.info("run: response length=%d", len(response))
                print(f"\nResponse:\n{response}")

            except KeyboardInterrupt:
                _logger.info("run: KeyboardInterrupt, exiting")
                break
